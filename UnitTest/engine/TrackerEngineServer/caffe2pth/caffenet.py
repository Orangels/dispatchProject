...
class PriorBox(nn.Module):
    ...
    def __repr__(self):
        return 'PriorBox(min_size=%f, max_size=%f, clip=%d, flip=%d, step=%d, offset=%f, variances=%s)' % (
            self.min_size, self.max_size, self.clip, self.flip, self.step, self.offset, self.variances)
    ...
    def load_weights(self, caffemodel):
        ...
        while i < layer_num:
            ...
            if ltype in ['Convolution', 'Deconvolution']:
                ...
                print('load weights %s' % lname)
                print(self.models[lname], nn.Sequential)
                print(self.models[lname][1].weight.data.shape)
    ...
    def create_network(self, net_info, input_width=None, input_height=None, input_channels=None, verbose=False):
        ...
        while i < layer_num:
            ...
            if ltype == 'Convolution':
                ...
                kernel_size = int(convolution_param['kernel_size']) if 'kernel_size' in convolution_param else int(
                    convolution_param['kernel_w'])
                # kernel_size = [int(convolution_param['kernel_w']),int(convolution_param['kernel_h'])]
                stride = int(convolution_param['stride']) if 'stride' in convolution_param else int(
                    convolution_param['stride_h'])
                pad = int(convolution_param['pad']) if 'pad' in convolution_param else int(convolution_param['pad_w'])
                ...
                if 'bias_term' in convolution_param and convolution_param['bias_term'] == 'false':
                    bias = False
                models[lname] = nn.Conv2d(channels, out_filters, kernel_size=kernel_size, stride=stride, padding=pad,
                ...
                models[lname] = MultiBoxLoss(num_classes, overlap_threshold, prior_for_matching, bkg_label, neg_mining,
                                             neg_pos, neg_overlap, use_gpu=True)
                blob_channels[tname] = 1
                blob_width[tname] = 1
                blob_height[tname] = 1
                i = i + 1
            ...
            if verbose:
                print('create %-20s (%4d x %4d x %4d) -> (%4d x %4d x %4d)' % (
                    lname, input_channels, input_height, input_width, output_channels, output_height, output_width))
