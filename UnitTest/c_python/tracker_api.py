...
def initialize_model_from_cfg():
    ...
    cfg.TEST.WEIGHTS = '/home/jwh/workspace/Pet-dev/ckpts/projects/fairmot/mot/fairmot-DLA34/all_dla34_trans-model-pet.pth'  # hard code
...
class ObjectApi:
    def __init__(self, frame_rate=25, cfg_file=None, opts=[]):
        ...
        self.model1 = initialize_model_from_cfg()
        self.model2 = initialize_model_from_cfg()
        # self.model3 = initialize_model_from_cfg()
        self.tracker0 = JDETracker(self.model0, frame_rate)  # frame rate???
        self.tracker1 = JDETracker(self.model1, frame_rate)  # frame rate???
        self.tracker2 = JDETracker(self.model2, frame_rate)  # frame rate???
        # self.tracker3 = JDETracker(self.model3, frame_rate)  # frame rate???
    ...
    def get_result(self, img0, img1, img2, img3):
        print("get_result")
        print(img0.shape)
        ret= []
        tlwhs0, ids0 = inference(self.tracker0, img0)
        # tlwhs1, ids1 = inference(self.tracker1, img1)
        # tlwhs2, ids2 = inference(self.tracker2, img2)
        # tlwhs3, ids3 = inference(self.tracker3, img3)
    ...
    def get_result1(self, img0):
        ret = []
        tlwhs, ids = inference(self.tracker1, img0)
        vis_im = plot_tracking(img0, tlwhs, ids)
        current = time.time()
        cv2.imwrite('./debug/1/' + str(int(current*10000000)) + '.png', vis_im)
        if len(tlwhs) == 0:
            return ret
        for label, det in zip(ids, tlwhs):
            ret.append(label)
    ...
    def get_result2(self, img0):
        ...
        vis_im = plot_tracking(img0, tlwhs, ids)
        current = time.time()
        cv2.imwrite('./debug/2/' + str(int(current * 10000000)) + '.png', vis_im)
        if len(tlwhs) == 0:
            return ret
        for label, det in zip(ids, tlwhs):
            ret.append(label)
        ...
        return ret
    #
    # def get_result3(self, img0):
    #     ret = []
    #     tlwhs, ids = inference(self.tracker3, img0)
    #     vis_im = plot_tracking(img0, tlwhs, ids)
    #     current = time.time()
    #     # cv2.imwrite('./debug/3/' + str(int(current * 10000000)) + '.png', vis_im)
    #     if len(tlwhs) == 0:
    #         return ret
    #     for label, det in zip(ids, tlwhs):
    #         ret.append(label)
